...
__align(4) uint8_t response_buff[MAX_PKT_SIZE];
    ...
    if ((gcmd == CMD_UPDATE_APROM) || (gcmd == CMD_UPDATE_DATAFLASH)) 
        ...
        if (TotalLen < srclen) {
            srclen = TotalLen;//prevent last package from over writing
				...
				g_packno++;
				outpw(response + 8, u8Check);
				PutString();
				//-------
				
        TotalLen -= srclen;
        WriteData(StartAddress, StartAddress + srclen, (uint32_t *)pSrc); //WriteData(StartAddress, StartAddress + srclen, (uint32_t*)pSrc);
        ReadData(StartAddress, StartAddress + srclen, (uint32_t *)pSrc_BK);
        StartAddress += srclen;
        LastDataLen =  srclen;

				// Swapping Buffer
				u8Check = 1;
				if( (memcmp(pSrc, pSrc_BK, srclen)) != 0)
					u8Check = 0;
