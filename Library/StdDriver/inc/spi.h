...
/**
  ...
  */
#define SPI_IS_BUSY(spi)	 ( (!SPI_GET_TX_FIFO_EMPTY_FLAG(spi)) && SPI_GET_RX_FIFO_EMPTY_FLAG(spi))
...
/**
  ...
  * @param[in]  spi The pointer of the specified SPI module.
  * @return     None.
  * @details    Reset transmit FIFO pointer and receive circuit.
  */
#define SPI_RESET_TXFIFO(spi)   ((spi)->FIFOCTL |= SPI_FIFOCTL_TXRST_Msk)
...
/**
  ...
  * @details    Reset receive FIFO pointer and receive circuit.
  */
#define SPI_RESET_RXFIFO(spi)   ((spi)->FIFOCTL |= SPI_FIFOCTL_RXRST_Msk)
...
#define SPI_I2SFORMAT_PCMB	   		(3<<SPI_I2SCTL_FORMAT_Pos)            	/*!< PCM standard mode data format */
