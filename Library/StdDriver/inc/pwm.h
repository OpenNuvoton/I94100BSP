...
/*---------------------------------------------------------------------------------------------------------*/
#define PWM_AUTORELOAD_COUNTER                   (0UL)      /*!< Auto-reload mode */
#define PWM_ONESHOT_COUNTER                      (1UL)      /*!< One-shot mode */
...
/**
 * @brief This macro set the PWM counter type
 * @param[in] pwm The pointer of the specified PWM module
 * @param[in] u32ChannelMask Combination of enabled channels. Each bit corresponds to a channel
 *                           Bit 0 represents channel 0, bit 1 represents channel 1...
 * @param[in] u32CounterType PWM counter type, valid values are:
 *              - \ref PWM_UP_COUNTER
 *              - \ref PWM_DOWN_COUNTER
 *							- \ref PWM_UP_DOWN_COUNTER
 * @return None
 * @details This macro is used to set the PWM counter type of specified channel(s).
 * \hideinitializer
 */
#define PWM_SET_COUNTER_TYPE(pwm, u32ChannelMask, u32CounterType) \
   do{ \
        int i; \
        for(i = 0; i < 6; i++) { \
            if((u32ChannelMask) & (1 << i)) \
                (pwm)->CTL1 = (((pwm)->CTL1 & ~(3UL << (2 * i))) | ((u32CounterType) << ( 2 * i))); \
        } \
    }while(0)

/**
 * @brief This macro set the PWM counter mode
 * @param[in] pwm The pointer of the specified PWM module
 * @param[in] u32ChannelMask Combination of enabled channels. Each bit corresponds to a channel
 *                           Bit 0 represents channel 0, bit 1 represents channel 1...
 * @param[in] u32CounterMode PWM counter type, valid values are:
 *              - \ref PWM_AUTORELOAD_COUNTER
 *              - \ref PWM_ONESHOT_COUNTER
 * @return None
 * @details This macro is used to set the PWM counter mode of specified channel(s).
 * \hideinitializer
 */
#define PWM_SET_COUNTER_MODE(pwm, u32ChannelMask, u32CounterMode) \
   do{ \
        int i; \
        for(i = 0; i < 6; i++) { \
            if((u32ChannelMask) & (1 << i)) \
                (pwm)->CTL1 = (((pwm)->CTL1 & ~(3UL << (2 * i))) | ((u32CounterMode) << ( 2 * i))); \
        } \
    }while(0)
