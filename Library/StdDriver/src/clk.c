...
uint32_t CLK_EnablePLL(uint32_t u32PllClkSrc, uint32_t u32PllFreq)
{
	...
	if((u32PllFreq <= FREQ_200MHZ) && (u32PllFreq >= FREQ_50MHZ))
	{
		...
		for(u32NO = 1UL; u32NO <= 4UL; u32NO++)
		{
			...
			if (u32NO != 3UL)
			{
				...
				// 200MHz <= FVCO = FOUT*NO <= 500MHz
				if((u32PllFreq >= FREQ_200MHZ) && (u32PllFreq <= FREQ_500MHZ))
				{
					for(u32NR = 2UL; u32NR <= 32UL; u32NR++)
					{
						...
						// 4MHz <= FREF = FIN/NR <= 8MHz
						// u32NR increase, u32Tmp will only get smaller
						if(u32Tmp < 4000000UL)
						{
							break;
						}
						...
						if(u32Tmp <= 8000000UL)
						{
							// u32NF will be round down to integer
							// To determine whether u32NF or u32NF+1 is more close to u32PllFreq.
							// FVCO = FREF*2*NF  =>  NF = (FVCO/2)/FREF
							u32NF = (u32PllFreq>>1)/u32Tmp;
							u32Tmp2 = u32Tmp*u32NF*2;
							u32Tmp3 = u32Tmp*(u32NF+1)*2;
							
							if(u32Tmp3-u32PllFreq < u32PllFreq-u32Tmp2)
							{
								u32NF = (u32NF+1);
								// Tmp3 = FVCO - Target FVCO
								u32Tmp3 = u32Tmp3-u32PllFreq;
							}
							else
							{
								// Tmp3 = Target FVCO - FVCO
								u32Tmp3 = u32PllFreq-u32Tmp2;
							}
							
							// Using FVCO deviation as index.
							if(u32Tmp3 < u32Min)
							{
								u32Min = u32Tmp3;
								u32MinNR = u32NR;
		...
		CLK->PLLCTL = u32CLK_SRC | ((u32MinNO - 1UL) << CLK_PLLCTL_OUTDIV_Pos) | ((u32MinNR - 1UL) << CLK_PLLCTL_INDIV_Pos) | (u32MinNF - 2UL);
	...
	else
	{
		...
		if(u32CLK_SRC == CLK_PLLCTL_PLLSRC_HXT)
		{
				CLK->PLLCTL = CLK_PLLCTL_98MHz_HXT;
		}
		else if(u32CLK_SRC == CLK_PLLCTL_PLLSRC_HIRC)
		{
			if(u32PllSrcClk == 49152000UL)
				CLK->PLLCTL = CLK_PLLCTL_98MHz_HIRC;
			else
