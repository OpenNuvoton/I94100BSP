...
uint32_t DMIC_SetSampleRate(DMIC_T* dmic,uint32_t u32SampleRate)
{
	...
	switch( dmic->CTL&DMIC_CTL_OSR_Msk )
	{
		...
		case DMIC_CTL_DOWNSAMPLE_256:		
			u32OSR = au16OSRTable[(dmic->CTL&DMIC_CTL_OSR_Msk)>>DMIC_CTL_OSR_Pos];
			u8K = 256;
		break;
		case DMIC_CTL_DOWNSAMPLE_100_50:
...
//	u32MainClock = u32SampleRate*(((u32SourceClock%u32SampleRate)!=0)? 256:500);
	u32MainClock = u32SampleRate * u8K;
